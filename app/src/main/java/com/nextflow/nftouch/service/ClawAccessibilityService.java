package com.nextflow.nftouch.service;
...
public class ClawAccessibilityService extends AccessibilityService {
    ...
    /** Financial / payment / banking apps — use MINIMAL mode to hide text content */
    private static final Set<String> PRIVACY_BLACKLIST = Set.of(
            "com.tencent.mm",                  // WeChat (chat + payment)
            "com.eg.android.AlipayGphone",     // Alipay
            "com.chinamworld.bocmbci",         // Bank of China
            "com.icbc",                         // ICBC
            "com.tencent.mobileqq",            // QQ
            "com.android.vending",             // Google Play
            "com.autonavi.minimap",            // Amap (location-sensitive)
            "com.tencent.wework",              // WeCom
            "com.xunmeng.pinduoduo",           // Pinduoduo
            "com.jingdong.app.mall"            // JD
    );

    /** Current screen mode: FULL (default) or MINIMAL (financial apps) */
    private enum TreeMode { FULL, MINIMAL }
    private TreeMode currentTreeMode = TreeMode.FULL;

    /** Max text/desc length before truncation — reduced for privacy */
    ...
    public String getScreenTree() {
        ...
        // Determine privacy mode based on app package
        currentTreeMode = TreeMode.FULL;
        CharSequence pkg = root.getPackageName();
        if (pkg != null) {
            for (String blocked : PRIVACY_BLACKLIST) {
                if (pkg.toString().contains(blocked)) {
                    currentTreeMode = TreeMode.MINIMAL;
                    break;
                }
            }
        }
    ...
    private void buildNodeTree(AccessibilityNodeInfo node, StringBuilder sb, int depth) {
        ...
        // Skip nodes not visible to the user (off-screen elements in scrollable containers)
        if (!node.isVisibleToUser()) {
            // Still traverse children since invisible parent does not mean all children are invisible
        ...
        // Check if the node is "meaningful" (has text/desc/interactive/scrollable/editable/progress/slider)
        ...
        if (isMeaningful) {
            ...
            // Simplify className: keep only the last segment (e.g. android.widget.TextView → TextView)
            ...
            // Privacy: skip text content for password fields
            if (node.isPassword()) {
                sb.append(" [password]");
                Rect bounds = new Rect();
                node.getBoundsInScreen(bounds);
                ...
                sb.append("\n");
                // Skip children of password nodes entirely
                return;
            }

            if (hasText && currentTreeMode == TreeMode.MINIMAL && !isInteractive) {
                // MINIMAL mode: hide text content, only show character count
                sb.append(" text=[").append(node.getText().length()).append("chars]");
            } else if (hasText) {
                // Truncate long text for privacy and output size control
                CharSequence text = node.getText();
                if (text.length() > MAX_TEXT_LENGTH) {
                    sb.append(" text=\"").append(text.subSequence(0, MAX_TEXT_LENGTH)).append("...\"");
                } else {
            ...
            if (hasDesc) {
                ...
                if (desc.length() > MAX_DESC_LENGTH) {
                    sb.append(" desc=\"").append(desc.subSequence(0, MAX_DESC_LENGTH)).append("...\"");
                } else {
        ...
        // Child depth: if current node is skipped (not meaningful), children stay at same level
    ...
    public boolean openRecentApps() {
        ...
        return sendKeyEvent(KeyEvent.KEYCODE_APP_SWITCH);
    }
