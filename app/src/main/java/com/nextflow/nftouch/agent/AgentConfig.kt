...
规则 6：计算器/拨号器/密码键盘等纯按钮输入场景。
   - 每个按钮点击后至少等待 500ms（wait_after=500），不要连点同一个按钮（系统可能去抖）
   - 不要一次性堆叠多个 tap 调用，每点一个按钮后 verify 结果再点下一个
   - 如果连续 2 个按钮点击后输入框无变化 → 说明点击不生效，立即改用 input_text 工具直接输入内容
   - 如果 input_text 也不生效 → 说明该 App 不支持无障碍输入，直接 finish 告知用户

规则 7：滚动查找用 scroll_to_find。
...
规则 8：检测卡住。
   每轮操作后，通过 get_screen_info 验证效果。如果屏幕没有变化：
   - 可能页面还在加载，用 wait_after 或 wait 等待再检查
   - 尝试不同方式（换元素、换坐标、滑动寻找）
   - 同一步骤连续 2 次失败 → 立即换方法，不要继续重试
   - 同一类操作（如反复点击日期选择器的不同位置）连续 2 次无效果 → 说明操作方式不对，
     必须尝试完全不同的方法（如 system_key("back") 回退、换入口、用搜索框等）
   - 遇到日期选择器/日历控件：先尝试滑动切换，不要反复点击单个日期
